...
normalize_fn = transforms.Normalize((0.48145466, 0.4578275, 0.40821073), (0.26862954, 0.26130258, 0.27577711))
resize_fn = transforms.Resize((256,256))
...
def create_dataset(dataset, config):
    ...
    transform_resize_norm = transforms.Compose([
        transforms.Resize((config['image_res'],config['image_res']),interpolation=Image.BICUBIC),
        normalize,
    ])
    ...
    if dataset=='pretrain':
    ...
    elif dataset=='re_gen_poison':  # clean dataset, used for train generator
        train_dataset = re_train_dataset(config['train_file'], train_transform, config['image_root'])
        val_dataset = re_eval_dataset(config['val_file'], test_transform, config['image_root'])  
        test_dataset = re_eval_dataset(config['test_file'], test_transform, config['image_root']) 
        return train_dataset, val_dataset, test_dataset  
    
    elif dataset=='re_gen_dataset':     # clean dataset, use for generate poisoning dataset
        ...
        train_dataset = re_train_dataset_with_poison(config['train_file'], train_transform_no_norm, normalize_fn,  config['image_root'], "/remote-home/songtianwei/research/unlearn_multimodal/datasets/poison/pascal/poison_data")
        val_dataset = re_eval_dataset(config['val_file'], test_transform, config['image_root'])  
        test_dataset = re_eval_dataset(config['test_file'], test_transform, config['image_root'])                
        return train_dataset, val_dataset, test_dataset  
