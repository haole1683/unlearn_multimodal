...
def train(generator, model, data_loader, optimizer, tokenizer, epoch, warmup_steps, device, scheduler):
    ...
    for batch_idx, (image, text, labels, idx) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        # 将image的倒数第4维度数据翻转（打乱图文对应关系）
        image_reverse = torch.flip(image, dims=[-4])
        
        # image_reverse = image，测试不翻转
        ...
        image_adv = torch.clamp(image_reverse + delta_im, min=0, max=1)
        ...
        loss_img_advImg = criterion_contrastive(clean_image_emb, adv_image_emb).mean()
        loss_text_advImg = criterion_contrastive(adv_image_emb, clean_text_emb).mean()
        
        adv_loss1 = loss_img_advImg
...
if __name__ == '__main__':
    ...
    output_dir = "./output/text_guide_target_gen_{}_{}".format(args.dataset, clip_model_str)
