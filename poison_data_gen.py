...
def generate_poison_dataset(generator, model, data_loader, tokenizer, device, config, args):
    ...
    with torch.no_grad():
        for batch_idx,(image, text, idx, anno) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
            ...
            # image_adv = torch.clamp(image, min=0, max=1)
...
def test_adversarial_attack(generator, model, data_loader, tokenizer, device, config, args):
    ...
    with torch.no_grad():
        for batch_idx,(image, text, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
            batch_size = len(image)
            image = image.to(device,non_blocking=True)   
            text_ids = tokenizer(text, truncate=True).to(device)
            # text = tokenizer(text, padding='longest', max_length=30, return_tensors="pt").to(device) 
...
def main(args, config):
    ...
    train_dataset, test_dataset, val_dataset = create_dataset('adversarial_test_dataset', config)
    ...
    test_adversarial_attack(generator, model, test_loader, tokenizer, device, config, args)
